...
# pokemons_primeiro_treinador = []
# pokemons_segundo_treinador = []
...
for player in range (0, 2): # Loop responsável por realizar o login dos treinadores.
    while True:
        ...
        if escolha == 1:
            ...
            playerr = entrar_conta()
            # todos = pegar_do_txt(f'usuarios/logins.txt')

            if player == 0:

                coisaspokemon1 = [] # nome, nivel, vida, atk, defesa, velocidade.
                coisastreinador1 = [] # potion, superpotion, hyperpotion, maxpotion

                with open(f'saves/{playerr}/pokemon.txt' , 'r') as arc:
                    for item in arc:
                        coisaspokemon1.append(item.replace('\n', ''))


                with open(f'saves/{playerr}/treinador.txt') as ar:
                    for item in ar:
                        coisastreinador1.append(item.replace('\n', ''))

                        
                nome1 = playerr.strip().capitalize()
                ...
                coisaspokemon2 = [] # nome, nivel, vida, atk, defesa, velocidade.
                coisastreinador2 = [] # potion, superpotion, hyperpotion, maxpotion


                with open(f'saves/{playerr}/pokemon.txt' , 'r') as aa:
                    for item in aa:
                        coisaspokemon2.append(item.replace('\n', ''))



                with open(f'saves/{playerr}/treinador.txt') as aaa:
                    for item in aaa:
                        coisastreinador2.append(item.replace('\n', ''))


                nome2 = playerr.strip().capitalize()
            ...
            salvar_informacoes(playerr, poke1.nivel, poke1.vida, poke1.atk, poke1.defesa, poke1.velocidade, treinador1.itens, escolhapokemon1, treinador1.pokedollar, poke1.vida)
        
        
        elif escolha == 3:
            playerr = excluir_conta()
...
while True: # Loop responsável pela batalha em si.
    if poke1.vida <= 0:
        ...
        salvar_informacoes(nome1, poke1.nivel, poke1.vida, poke1.atk, poke1.defesa, poke1.velocidade, treinador1.itens, escolhapokemon1, treinador1.pokedollar, coisaspokemon1[6])
        salvar_informacoes(nome2, poke2.nivel, poke2.vida, poke2.atk, poke2.defesa, poke2.velocidade, treinador2.itens, escolhapokemon2, treinador2.pokedollar, coisaspokemon2[6])
        break
